...
describe ASM::WsMan do
  ...
  describe "when parsing nicview with disabled 57800 and dual-port slot nic" do
    ...
    let(:nic_view_response) { SpecHelper.load_fixture("wsman/nic_view.xml") }
  ...
  describe "#invoke_service" do
    let(:url) { "http://rspec.com/test"}

    it "should invoke invoke and parse the command" do
      expected = {:return_value => "0", :foo => "foo"}
      ASM::WsMan.expects(:invoke).with(endpoint, "RspecCommand", url, :logger => logger, :props => {}).returns("<rspec />")
      ASM::WsMan.expects(:parse).with("<rspec />").returns(expected)
      expect(ASM::WsMan.invoke_service(endpoint, "RspecCommand", url, :logger => logger)).to eq(expected)
    end

    it "should invoke the command with its arguments" do
      ASM::WsMan.expects(:invoke)
        .with(endpoint, "RspecCommand", url, :props => {"Foo" => "bar"}, :logger => logger)
        .returns("<rspec />")
      ASM::WsMan.expects(:parse).with("<rspec />")
      ASM::WsMan.invoke_service(endpoint, "RspecCommand", "http://rspec.com/test",
                                :required_params => [:foo], :foo => "bar", :logger => logger)
    end
    ...
    it "should ignore unknown options" do
      ...
      ASM::WsMan.expects(:parse).with("<rspec />")
      ASM::WsMan.invoke_service(endpoint, "RspecCommand", "http://rspec.com/test",
                                :required_params => [:foo], :foo => "bar", :ignored => "foo", :logger => logger)
    end

    it "should fail if one of :required_params not supplied" do
      expect do
        ASM::WsMan.invoke_service(endpoint, "RspecCommand", "http://rspec.com/test", :required_params => [:foo])
      end.to raise_error("Missing required parameter(s) for RspecCommand: foo")
    end

    it "should fail if one of :url_params not supplied" do
      expect do
        ASM::WsMan.invoke_service(endpoint, "RspecCommand", "http://rspec.com/test", :url_params => [:foo])
      end.to raise_error("Missing required parameter(s) for RspecCommand: foo")
    end

    it "should add query parameters to url with query params" do
      ASM::WsMan.expects(:invoke).with(endpoint, "RspecCommand", "http://rspec.com/test?InstanceID=rspec-instance-id",
                                       :logger => logger, :props => {})
      ASM::WsMan.expects(:parse)
      ASM::WsMan.invoke_service(endpoint, "RspecCommand", "http://rspec.com/test",
                                :url_params => [:instance_id], :instance_id => "rspec-instance-id", :logger => logger)
    end

    it "should add query parameters to url without query params" do
      ASM::WsMan.expects(:invoke).with(endpoint, "RspecCommand", "http://rspec.com/test?foo=bar&InstanceID=rspec-instance-id",
                                       :logger => logger, :props => {})
      ASM::WsMan.expects(:parse)
      ASM::WsMan.invoke_service(endpoint, "RspecCommand", "http://rspec.com/test?foo=bar",
                                :url_params => [:instance_id], :instance_id => "rspec-instance-id", :logger => logger)
    end

    it "should fail if the ReturnValue does not match" do
      expected = {:return_value => "2", :message => "Stuff broke"}
      ASM::WsMan.expects(:invoke).with(endpoint, "RspecCommand", "http://rspec.com/test", :logger => logger, :props => {}).returns("<rspec />")
      ASM::WsMan.expects(:parse).with("<rspec />").returns(expected)
      expect do
        ASM::WsMan.invoke_service(endpoint, "RspecCommand", "http://rspec.com/test", :return_value => "0", :logger => logger)
  ...
  describe "#run_deployment_job" do
    ...
    let(:invoke_options) do
      options.merge(:required_params => [:ip_address, :share_name, :share_type, :image_name],
    ...
    it "should poll for LC ready, invoke command and poll job" do
      ASM::WsMan.expects(:poll_deployment_job).with(endpoint, "rspec-job", invoke_options)
